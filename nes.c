...
u8 nes_read8(struct nes *nes, u16 addr)
{
    switch(addr >> 13)
    {
        ...
        case 2: printf("NOT IMPLEMENTED IO/APU READ: %04x\n", addr); return 0;
        case 3: printf("NOT IMPLEMENTED READ %04x\n", addr); nes->cpu.uoc = true; return 0;
...
void nes_write8(struct nes *nes, u16 addr, u8 value)
{
    switch(addr >> 13)
    {
        ...
        case 2:
            if (addr == 0x4014)
            {
                u16 baseaddr = nes->ppu.registers.OAMADDR << 8 | 00;
                for (int i = 0; i < 256; i++)
                    nes->ppu.oam[i] = nes_read8(nes, baseaddr);
                return;
            }
            printf("NOT IMPLEMENTED IO/APU WRITE: %04x\n", addr);
            break;
        case 3: printf("NOT IMPLEMENTED CART WRITE %04x\n", addr); nes->cpu.uoc = true; break;
